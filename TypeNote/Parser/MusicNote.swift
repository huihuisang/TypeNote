...
struct MusicNote: Codable, Equatable, Identifiable {
    ...
    /// Return a copy with duration multiplied by the given ratio.
    func scaled(durationBy ratio: Double) -> MusicNote {
        MusicNote(midiNote: midiNote, duration: duration * ratio, velocity: velocity, isRest: isRest)
    }
